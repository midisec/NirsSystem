...
class NirspyPredictor(object):
    ...
    def model_load(self, pre_way, model_name):
        with open("{}/models/{}/{}_{}.pkl".format(self.modelfolder, model_name, model_name, pre_way), "rb") as f:
            model = pickle.load(f)
            return model

    def predict(self, data, save_path=None):
        # 评估所有模型，寻找最优组合
        pre_way, model = self.assessment_handler()

        # 找到最优，进行预处理，预测
        data = self.pretreatment_handler(data, pre_way)

        # 加载模型，预测
        model = self.model_load(pre_way, model)
        predictor_result = model.predict(data)
        ...
        # print(len(predictor_result))



if __name__ == '__main__':
    data = pd.read_csv(r"E:\大学生创新创业\demo\data.csv")
    model = NirspyPredictor('default')
